...
boolean castInt(s_var *v){
	switch(v->type){
		...
		case S_CHAINE: v->type = S_ENT;v->value.ve = (v->value.vs == NULL)?0:parseInt(v->value.vs);free(v->value.vs);break;
		default: return False;
	}
	return True;
}
...
boolean castFloat(s_var *v){
	switch(v->type){
		...
		case S_CHAINE: v->type = S_FLOT;v->value.vf = (v->value.vs == NULL)?0.:parseFloat(v->value.vs);free(v->value.vs);break;
		default: return False;
	}
	return True;
}
...
boolean castChar(s_var *v){
	switch(v->type){
		...
		case S_CHAINE: v->type = S_CAR;v->value.vc = (v->value.vs == NULL)?'\0':v->value.vs[0];free(v->value.vs);break;
		default: return False;
	}
	return True;
}
...
boolean castBool(s_var *v){
	switch(v->type){
		...
		case S_CHAINE: v->type = S_BOOL;v->value.vb = (v->value.vs == NULL || strcmp(v->value.vs, "") == 0)?True:False;free(v->value.vs);break;
		default: return False;
	}
	return True;
}
